...
- JSP 페이지로 서블릿 클래스르 ㄹ만들 때 _jspService() 메서드 안에 생성되는 객체이다.
- 빌트인 객체
  request:HttpServletRequest    - _jspService()의 파라미터
  request:HttpServletResponse   - _jspService()의 파라미터
  ...
  exception:Throwable           - _jspService()의 로컬 변수
    - JSP의 페이지의 isErrorPage속성 값을 true로 설정하면 이 변수를 사용할 수 있다.
</pre>
...
// 위의 9개의 변수는 JSP 페이지에 ㅂ조냊하는 변수이기 때문에 컴파일 오류가 발생하지 않는다.
// 그러나 다음 "ok와 같이 임의의 변수는 존재하지 않기 때문에 
// out.println(ok);
%>
