...
public class MyAdvice {
    ...
    @Before("execution(* ex11.step7.ServiceImpl.addPhoto(..)) and args(f)")
    public void before(String f) {
        System.out.println("MyAdvice.before(): " + f);
    }
    
    @AfterReturning(pointcut="execution(* ex11.step6.ServiceImpl.addPhoto(..))",
                returning="rt")
    public void afterReturning(Object rt) {
        System.out.println("MyAdvice.afterReturning(): " + rt.toString());
    }
    
    @AfterThrowing(pointcut="execution(* ex11.step6.ServiceImpl.addPhoto(..))",
            throwing="err")
