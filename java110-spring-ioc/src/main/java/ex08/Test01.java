// 의존 객체 자동 주입 : @Autowired 애노테이션 과 BeanPostProcessor
...
//    단  String을 java.sql.Date으로 바꾸기 위해 커스텀 프로퍼티 에디터를 장착하는 것 처럼
// 
// => @Autowired 애노테이션 처리기를 장착하는 방법
//    AutowiredAnnotationBeanPostProcessor 클래스의 객체를 등록하라!
//    그냥 등록만 하면 된다.
//    그러면 스프링 IoC 컨테이너가 알아서 이 갹체를 사용하여 @Autowired를 처리한다.

// BeanPostProcessor ?
// => 스프링 IoC 컨테이너는 객체를 생성한 후  BeanPostProcessor를 실행한다
