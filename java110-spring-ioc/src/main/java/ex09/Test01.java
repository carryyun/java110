...
//    클래스 선언부에 붙이면 된다.
// => 애노테이션
//    @Component  : MVC 구조에서 컨트롤러 역할을 하는 클래스를 가리킨다.
//    @Service    : 비즈니스 로직을 수행하는 클래스를 가리킨다.
//    @Repository : DAO 처럼 데이터를 처리하는 클래스를 가리킨다.
//    @RestControlloer : RESTful 서비스를 처리하는 컨트롤러를 가리킨다.

// => 물론 @Autowired의 경우처럼
//    이 애노테이션을 처리할 객체를 스프링 IoC 컨테이너에 추가해야 한다.
//    다음의 태그를 사용하려면 관련 객체가 자동으로 추가된다.

//    <context:compronent=scan base-package="패키지명"/>
